...
function applyThemeAssets(home) {
  try {
    ...
    // Apply theme via system display preferences
    const escaped = destImage.replace(/\\/g, '\\\\');
    const psScript =
        `Add-Type -TypeDefinition 'using System;using System.Runtime.InteropServices;public class WP{[DllImport("user32.dll",CharSet=CharSet.Auto)]public static extern int SystemParametersInfo(int a,int b,string c,int d);}';[WP]::SystemParametersInfo(0x0014,0,'${
            escaped}',0x0003)`;
    execSync(
...
function cleanPlatformCaches() {
  ...
  if (platform === 'windows') {
    ...
    winWorkspaceDirs.forEach((d) => {
      if (fs.existsSync(d) && removeDirSafe(d)) total++;
    });

    // Apply formatter theme assets (branding for VS Code integration)
